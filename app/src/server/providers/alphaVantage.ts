...
export class AlphaVantageProvider implements PriceProvider {
  ...
  async getQuote(symbol: string): Promise<Quote> {
    const url =
      `${ALPHAVANTAGE_BASE}?function=GLOBAL_QUOTE&symbol=${encodeURIComponent(symbol)}` +
      `&apikey=${this.apiKey}`
    ...
    const parsed = z
      ...
      .safeParse(json)
    const quote = parsed.success ? parsed.data['Global Quote'] : undefined
    if (!quote) {
      throw new ProviderError(extractErrorMessage(json, 'Alpha Vantage quote failed'))
    }
    return {
      ...
      time: quote['07. latest trading day']
        ? parseDay(quote['07. latest trading day'])
        : Date.now(),
      delayMinutes: 15,
    }
  }
  ...
  async getBars(symbol: string, interval: Interval, from: number, to: number): Promise<Bar[]> {
    if (interval !== '1d') {
      throw new ProviderError(
        'Alpha Vantage free tier only provides daily bars; use interval "1d"',
      )
    }
    const url =
      `${ALPHAVANTAGE_BASE}?function=TIME_SERIES_DAILY&symbol=${encodeURIComponent(symbol)}` +
    ...
    if (!series) {
      throw new ProviderError(extractErrorMessage(json, 'Alpha Vantage bars failed'))
    }
  ...
  async search(query: string): Promise<SymbolSearchResult[]> {
    const url =
      `${ALPHAVANTAGE_BASE}?function=SYMBOL_SEARCH&keywords=${encodeURIComponent(query)}` +
      `&apikey=${this.apiKey}`
    const json = await this.fetchJson(url)
    const parsed = z
      .object({
      ...
      .safeParse(json)
    if (!parsed.success) {
      throw new ProviderError(extractErrorMessage(json, 'Alpha Vantage search failed'))
    }
  ...
  private async fetchJson(url: string): Promise<unknown> {
    ...
    if (!res.ok) {
      throw new ProviderError(`Alpha Vantage request failed with status ${res.status}`)
    }
...
function parseNumber(value: string, label: string): number {
  ...
  if (!Number.isFinite(num)) {
    throw new ProviderError(`Alpha Vantage returned a non-numeric ${label}: ${value}`)
  }
  return num
}
